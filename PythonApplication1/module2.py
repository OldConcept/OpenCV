...
while(True):
    ...
    cnts1,h1 = cv2.findContours(mask1.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    cnts2,h2 = cv2.findContours(mask2.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    cnts3,h3 = cv2.findContours(mask3.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    if (len(cnts1) > 0 and len(cnts2) > 0  and len(cnts3) > 0) :
        c1 = max(cnts1, key=cv2.contourArea)#找到具有最大面积的轮廓

        rect1 = cv2.minAreaRect(c1)#获取最小外接矩形
        box1 = np.int0(cv2.boxPoints(rect1))                                   #获取最小外接矩形四个顶点的坐标
        box1[0][0] = box1[0][0] 
        box1[1][0] = box1[1][0] 
        box1[2][0] = box1[2][0] 
        box1[3][0] = box1[3][0] 
...
#    Uart_buf = bytearray([0x55,0xAA,0x30,Postion_y>>8,Postion_y & 0x00ff,                         (angle2 & 0xff00)>>8,  angle  & 0x00ff,0, 0, 0, 0,0xAA])
#    com.send(Uart_buf)
    cv2.imshow("循黑线3段",frame)
